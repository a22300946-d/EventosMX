/**
 * models/Solicitud.js  (ACTUALIZADO)
 * ─────────────────────────────────────────────────────────────────────────────
 * Cambio: agrega el método estático `resetearFlagsNotificacion` que
 * timerService y solicitudController utilizan para inicializar los flags
 * de las nuevas solicitudes.
 *
 * Todo lo demás permanece idéntico a la versión original.
 */
...
class Solicitud {
  ...
  // ── Obtener solicitud por ID (con datos de cliente y proveedor) ───────────
  ...
  static async actualizarEstado(id_solicitud, nuevoEstado, id_usuario, rol) {
    let query = `UPDATE Solicitud SET estado = $1`;
  ...
  // ── Marcar notificación como enviada (método legacy) ─────────────────────
  ...
  // ★ ── Resetear flags de notificación al crear una solicitud nueva ─────────
  static async resetearFlagsNotificacion(id_solicitud) {
    const query = `
      UPDATE Solicitud
      SET recordatorio_proveedor_enviado = false,
          notificacion_cliente_enviada   = false
      WHERE id_solicitud = $1
  ...
  // ── Obtener solicitudes pendientes sin respuesta (método legacy) ──────────
  ...
  static async responderConPropuesta(id_solicitud, id_proveedor, propuesta) {
    const { mensaje_respuesta, precio_propuesto, detalles_servicio, fecha_disponible } = propuesta;
